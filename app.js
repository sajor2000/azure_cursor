...
// Reference data for medication classes
const medicationReference = {
  ...
  // ... (other classes omitted for brevity, but should be included in real code)
};
...
app.post('/api/agent', async (req, res) => {
  ...
  if (!insurance_pbm || !preference || !medication_class) {
    return res.status(400).json({
      error: "Missing required fields: insurance_pbm, preference, medication_class"
    });
  }
  try {
...
app.get('/', (req, res) => {
  res.send('<h1>Azure AI Agent Web App</h1><p>POST to /api/agent with {"insurance_pbm": "...", "preference": "...", "medication_class": "..."} to interact with the agent.</p>');
});
